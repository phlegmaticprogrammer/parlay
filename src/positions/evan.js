...
class Peer {
    ...
    updateAnimations(seconds) {
        ...
        const visit = (node, x, y) => {
            ...
            if (!info) {
                const oldParent = node.parent && this.animatedTreeNodes.get(node.parent.id)
                info = oldParent ? { x: oldParent.x, y: oldParent.y, width: 0 } : { x, y, width: 0 }
            }
            info.x += (x - info.x) * t
            info.y += (y - info.y) * t
    ...
    draw(context) {
        ...
        for (const node of this.tree.nodes.values()) {
            ...
            if (!edges.size) {
                ...
                for (let i = 0; i < sorted.length; i++) {
                    ...
                    context.beginPath()
                    context.arc(offsetX + radius * sign, offsetY, radius, 0, 2 * Math.PI, false)
                    context.fill()
        ...
        const visit = (node) => {
            ...
            if (node.id === this.draggingID) {
                ...
                for (const [id, parent] of this.tree.nodes) {
                    ...
                    if (id !== node.id && distanceBetween(targetX, targetY, treeX + otherX, treeY + otherY) < 15) {
                ...
                context.rotate(Math.atan2(treeX + nodeX - targetX, targetY - treeY - nodeY))
                context.beginPath()
                context.moveTo(0, 0)
                context.lineTo(-5, -10)
                context.lineTo(5, -10)
        ...
        const treeX = Math.round(w * (x ? 2 : 3) / 5) - this.animatedTreeNodes.get(this.tree.root.id).width / 2
        const treeY = 50
        context.strokeStyle = textColor
        context.lineWidth = 1
...
setTimeout(() => { peers[0].db.set('abk0980oz3', peers[0].tree.root.id, 0); peers[0].undoRedo.undoHistory = [] }, 100)
setTimeout(() => { peers[0].db.set('tkr24y7uli', peers[0].tree.root.id, 0); peers[0].undoRedo.undoHistory = [] }, 200)
setTimeout(() => { peers[0].db.set('dopsc4mbaup', 'abk0980oz3', 0); peers[0].undoRedo.undoHistory = [] }, 300)
setTimeout(() => { peers[0].db.set('p97h88b4lgr', 'abk0980oz3', 0); peers[0].undoRedo.undoHistory = [] }, 400)
setTimeout(() => { peers[0].db.set('akumsplmgeq', 'tkr24y7uli', 0); peers[0].undoRedo.undoHistory = [] }, 500)
setTimeout(() => { peers[0].db.set('yas5wscaid9', 'tkr24y7uli', 0); peers[0].undoRedo.undoHistory = [] }, 600)
    ...
    for (const b of peers) {
        if (a !== b) {
            a.db.afterApply(({ op, origin }) => {
                if (origin !== 'remote') packets.push({ op, from: a.id, to: b.id, life: 0.05, lifetime: 0.5 })
            })
        }
    }
}
...
function draw() {
    const width = isMobile ? Math.min(600, document.body.clientWidth - 2 * 30) : Math.min(1200, document.body.clientWidth - 2 * 50)
    ...
    for (const packet of packets) {
        ...
        context.fillStyle = textColor
        context.beginPath()
        context.arc(from.peerX + (to.peerX - from.peerX) * t, from.peerY + (to.peerY - from.peerY) * t, 5, 0, 2 * Math.PI, false)
        context.fill()
    }
    context.restore()
...
function tick() {
    ...
    const seconds = currentFrame - previousFrame < 500 ? (currentFrame - previousFrame) / 1000 : 0
...
addEventListener('touchstart', e => {
    if (e.touches.length === 1) {
        ...
        for (const button of buttons) {
            if (distanceBetween(x, y, button.x, button.y) < button.radius * (isMobile ? 1.5 : 1)) {
                e.preventDefault()
                if (button.dragAction) {
                    button.dragAction.start()
                    const onmove = e => {
